...
def load_data(data_dir):
    ...
    energy = pd.read_csv(os.path.join(data_dir, 'client_1.csv'), parse_dates=['Data End Time'])
    energy = energy.drop(columns=['Meter No.', 'Data Start Time'])
    energy = energy.rename(columns={'Active energy import A+(1.8.0)': 'load'})

    # Reindex the dataframe such that the dataframe has a record for every time point
    # between the minimum and maximum timestamp in the time series. This helps to 
    # identify missing time periods in the data (there are none in this dataset).

    energy.index = energy['Data End Time'] # type: ignore
    energy = energy.reindex(pd.date_range(min(energy['Data End Time']),
                                          max(energy['Data End Time']),
                                          freq='15min'))
    energy = energy.drop('Data End Time', axis=1)
